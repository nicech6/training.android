package com.xpleemoon.player.controller
...
import com.xpleemoon.player.exception.PlaybackRenderException
import com.xpleemoon.player.exception.PlaybackSourceException
import com.xpleemoon.player.exception.PlaybackUnexpectedException
...
internal class PlayControllerImpl(private val player: SimpleExoPlayer) : PlayController {
    ...
    override var supportRepeatModes: IntArray = intArrayOf(Player.REPEAT_MODE_OFF, Player.REPEAT_MODE_ONE, Player.REPEAT_MODE_ALL)
    /**
     * 循环模式
    ...
    override fun play() {
        when (player.playbackState) {
            ...
            Player.STATE_ENDED -> controlDispatcher.dispatchSeekTo(player, player.currentWindowIndex, C.TIME_UNSET)
        }
...
//                        && !player.isCurrentWindowSeekable)) {
    ...
    private fun updateProgress() {
        ...
        playbackListeners?.forEach { it.onPlaybackProgressChanged(currentPositionMS, currentBufferedPositionMS, currentDurationMS) }
