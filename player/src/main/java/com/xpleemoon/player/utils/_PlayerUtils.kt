...
internal fun newDefaultPlayerInstance(context: Context,
...
internal fun newAudioPlayerInstance(context: Context,
                                    trackSelector: TrackSelector = newDefaultTrackSelector())
        : SimpleExoPlayer {
    val renderersFactory: RenderersFactory = object : DefaultRenderersFactory(context) {
         ...
         * 空实现的目的：禁用视频渲染
         */
        override fun buildVideoRenderers(context: Context?, drmSessionManager: DrmSessionManager<FrameworkMediaCrypto>?, allowedVideoJoiningTimeMs: Long, eventHandler: Handler?, eventListener: VideoRendererEventListener?, extensionRendererMode: Int, out: ArrayList<Renderer>?) {
        }
    }
